...
<html>
  ...
  <div class="container" id="navbarcima">
      ...
      <?php 
        ...
        $comandoSql = "select * from tb_doacao where id_doacao = '$doacaoEscolhida'";
        $resultado=mysqli_query($con,$comandoSql);
        $doacao=mysqli_fetch_array($resultado);

        $idong = $doacao['cod_ong'];
      ...
      <form class="row g-3" action="alterar.php" method="post">
        <div class="row my-2">
          <div class="col-md-2">
            <label for="comprimento" class="form-label">Comprimento do cabelo doado</label>
            <div class="input-group">
              <input type="number" min="20" max="99" step="1" class="form-control" id="comprimento" name="comprimento" onkeypress="$(this).mask('00')" value="<?=$doacao["cm_doacao"]?>">
          ...
          <label for="tipo" class="form-label">Tipo de cabelo</label>
            <select id="tipo" class="form-select" name="tipo" required>
              <?php if($doacao["tipo_cabelo"] == "Liso"){?>
              <option selected>Liso</option>
              <option>Ondulado</option>
              <option>Cacheado</option>
              <option>Crespo</option>

              <?php } else if($doacao["tipo_cabelo"] == "Ondulado"){?>
              <option>Liso</option>
              <option selected>Ondulado</option>
              <option>Cacheado</option>
              <option>Crespo</option>

              <?php } else if($doacao["tipo_cabelo"] == "Cacheado"){?>
              <option>Liso</option>
              <option>Ondulado</option>
              <option selected>Cacheado</option>
              <option>Crespo</option>
        ...
        <div class="row my-2">
          <div class="col-2">
            ...
            <input type="text" class="form-control" id="cor" name="cor" value="<?=$doacao["cor_cabelo"]?>"required>
          </div>
          <div class="col-md-2">
          <label for="forma" class="form-label">Forma de doação</label>
            <select id="forma" class="form-select" name="forma" required>

              <?php if($doacao["forma_doacao"] == "Correios"){?>
              <option selected>  Correios </option>
              <option>  Entregue em ponto de coleta </option>

              <?php } else {?>
              <option>  Correios </option>
              <option selected>  Entregue em ponto de coleta </option>
              <?php } ?>  
        ...
        <div class="row my-2">
          <div class="col-md-2">
            ...
            <input class="form-control" type="date" id="data" min="2020-12-30" value="<?=$doacao["data_doacao"]?>" name="data">
          </div>
        </div>

        <input type="hidden" name="id" value="<?php echo $doacao["id_doacao"];?>"></input>
        <div class="col-12">
          <button type="submit" class="btn btn-warning"> Alterar doação </button>
        </div>
      </form>
      </div>
      </main>      
